...
App::router()->prefix('/h-plugin-creator', function() {
    App::router()->auth(App::session()->isAllowed('h-plugin-creator.manage-collections'), function() {
        App::router()->get('h-plugin-creator-manage-collections', '/collections', array(
            'action' => 'AdminController.index'
        ));

        App::router()->any('h-plugin-creator-manage-collection', '/collections/{collectionId}', array(
            'where' => array(
                'collectionId' => '\d+'
            ),
        ...
        App::router()->get('h-plugin-creator-edit-collection-field', '/collections/fields', array(
            'action' => 'AdminController.editField'
        ));
    });

    // The route to access the list of the records of a defined collection
    App::router()->get('h-plugin-creator-records', '/records/{collection}', array(
        'auth' => function($route) {
            if(!$route->getData()) {
                return true;
            }
    ...
    // Delete a record
    App::router()->delete('h-plugin-creator-delete-record', '/records/{collection}/{recordId}/delete', array(
        'where' => array(
            'collection' => '\w+',
            'recordId' => '\d+'
        ),
    ...
    App::router()->any('h-plugin-creator-record', '/records/{collection}/{recordId}', array(
        'where' => array(
            'collection' => '\w+',
            'recordId' => '\d+'
        ),
